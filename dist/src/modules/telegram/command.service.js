...
function formatTask(task) {
    const status = { TODO: "⚪ TODO", WORKING: "🔵 WORKING", BLOCKED: "🔴 BLOCKED", DONE: "🟢 DONE", CANCELLED: "⚫ CANCELLED" }[task.status] ?? task.status;
...
export async function handleCommand(userId, text) {
    ...
    if (command === "progress") {
        ...
        try {
            ...
            if (error instanceof Error && error.message === "TASK_CLOSED")
                return "Task tersebut sudah ditutup.";
            throw error;
        }
    }
    if (command === "block") {
        if (!args)
            return "Format: block Alasan\nContoh: block Menunggu API dari client";
        const task = await getLatestTask(userId);
        if (!task)
            return "📭 Tidak ada task aktif.";
